...
const ConsumersDales = () => {
  ...
  const updateBaqaya = async (consumerName, totalAmount) => {
    try {
      ...
      const response = await fetch(
        `https://api.maherdairy.com/consumerkhata?name=${encodeURIComponent(
          consumerName
        )}`
      );
      ...
      const updateResponse = await fetch(
        `https://api.maherdairy.com/consumerkhata/${matchingConsumer._id}`,
        {
          method: "PUT",
          headers: {
    ...
    try {
      ...
      if (editIndex >= 0) {
        ...
        response = await fetch(
          `https://api.maherdairy.com/consumerssale/${expenseId}`,
          {
            method: "PUT",
    ...
    if (isConfirmed && deleteIndex != null) {
      ...
      if (expense && expense._id) {
        try {
          ...
          const updateResponse = await fetch(
            `https://api.maherdairy.com/consumerkhata/${consumer._id}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
          ...
          const deleteResponse = await fetch(
            `https://api.maherdairy.com/consumerssale/${expense._id}`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
